...
def read_dataset(dirs_path, pose):
    ...
    for d in dirs:
        ...
        if d == dirs[0]:
            ...
            x_train_file_name = np.array(data["x_train_file_name"])
            y_train_file_name = np.array(data["y_train_file_name"])
        else:
            ...
            x_train_file_name = np.hstack((x_train_file_name, np.array(data["x_train_file_name"])))
            y_train_file_name = np.hstack((y_train_file_name, np.array(data["y_train_file_name"])))

        angle_dif = np.hstack((v_delta.reshape(-1, 1), h_delta.reshape(-1, 1)))
    return x_train_L / 255, y_train_L / 255, x_train_R / 255, y_train_R / 255, feature_point_R / 1.0, feature_point_L / 1.0, angle_dif / 1.0,x_train_file_name, y_train_file_name


def input2data(faces_folder_path, eyes_array):
    # usage
    ...
    for i in range(0, feature_point_num):
        ...
        right_eye_coord_global[i][0] = eyes_array[0][i][0]
        right_eye_coord_global[i][1] = eyes_array[0][i][1]
    for i in range(0, feature_point_num):
        # print("1,"+ str(i) + "," + str(shape.part(left_eye_index[i]).x) + "," + str(shape.part(left_eye_index[i]).y))
        left_eye_coord_global[i][0] = eyes_array[1][i][0]
        left_eye_coord_global[i][1] = eyes_array[1][i][1]
    grace_width = 5
    grace_height = 4
    ...
    R_width = int((R_xmax - R_xmin) / 2 * grace_ratio + 0.5) + grace_width
    ...
    L_width = int((L_xmax - L_xmin) / 2 * grace_ratio + 0.5) + grace_width
