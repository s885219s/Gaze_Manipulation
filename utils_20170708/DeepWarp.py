def create_model(input_img, input_agl, input_ef, conf, cfw_only = False, is_bias= False):    
    from keras.models import Model
    from keras.layers import Lambda, Dense, Conv2D, Activation, BatchNormalization, concatenate, add, AveragePooling2D
    from keras.layers.advanced_activations import LeakyReLU
    import tensorflow as tf
    from transformation import apply_transformation, trans_angle, apply_light_weight,apply_light_weight_single
    '''inputes'''
    ...
    locnet = AveragePooling2D(pool_size=(2, 2), strides=(2, 2))(input_datas)

    locnet = Conv2D(16, (5, 5),padding='same', use_bias=is_bias)(locnet)
    locnet = BatchNormalization()(locnet)
    locnet = Activation('relu')(locnet)

    locnet = Conv2D(32, (3, 3),padding='same', use_bias=is_bias)(locnet)
    locnet = BatchNormalization()(locnet)
    locnet = Activation('relu')(locnet)
    ...
    locnet = Conv2D(32, (1, 1),padding='same', use_bias=is_bias)(light_feature_1)
    locnet = BatchNormalization()(locnet)
    locnet = Activation('relu')(locnet)
    ...
    D_c_flow = Lambda(lambda image: tf.image.resize_images(image,
                                                           (conf.height, conf.width),
                                                           method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                                           output_shape=(conf.height, conf.width, 2),
                                                           name = 'D_c_flow')(locnet)
    ...
    locnet2 = Conv2D(16, (5, 5),padding='same', use_bias=is_bias)(D_fin_input)
    locnet2 = BatchNormalization()(locnet2)
    locnet2 = Activation('relu')(locnet2)
    # locnet2 = LeakyReLU(alpha=0.02)(locnet2)
    
    locnet2 = Conv2D(32, (3, 3),padding='same', use_bias=is_bias)(locnet2)
    locnet2 = BatchNormalization()(locnet2)
    locnet2 = Activation('relu')(locnet2)
    ...
    locnet2 = Conv2D(32, (1, 1),padding='same', use_bias=is_bias)(light_feature_2)
    locnet2 = BatchNormalization()(locnet2)
    locnet2 = Activation('relu')(locnet2)
    ...
    '''lcm'''
    light_feature_1_res = Lambda(lambda image: tf.image.resize_images(image,
                                                                      size=(conf.height, conf.width),
                                                                      method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                                                      output_shape=(conf.height, conf.width, 32))(light_feature_1)

    light_feature_input = concatenate([light_feature_1_res, light_feature_2], axis = 3, name = 'light_feature_input')
    
    locnet3 = Conv2D(8, (1, 1),padding='same', use_bias=is_bias)(light_feature_input)
    locnet3 = BatchNormalization()(locnet3)
    locnet3 = Activation('relu')(locnet3)

    locnet3 = Conv2D(8, (1, 1),padding='same', use_bias=is_bias)(locnet3)
    locnet3 = BatchNormalization()(locnet3)
    locnet3 = Activation('relu')(locnet3)

    locnet3 = Conv2D(2, (1, 1),padding='same')(locnet3)
